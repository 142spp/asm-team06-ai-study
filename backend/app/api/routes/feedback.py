...
from app.feedback.db import load_user_preferences, save_candidate_log, save_user_preference
...
class ConfirmRequest(BaseModel):
    ...
    candidates: list[Candidate]  # 사용자가 앞으로 적용하기로 선택한 후보
...
async def confirm_preference(body: ConfirmRequest) -> ConfirmResponse:
    if body.action == "dismiss":
        return ConfirmResponse(session_id=body.session_id, saved=False)
    ...
    return ConfirmResponse(session_id=body.session_id, saved=True)
